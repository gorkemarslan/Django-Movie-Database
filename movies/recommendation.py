...
def recommender(user: get_user_model()) -> List[str]:
    ...
    movie_id = movie[movie['title'].isin(input_movie['title'].tolist())]
    ...
    # Sort user_subset_group so that users who have similar movie tast will have higher priority
    ...
    recommendation_list.drop(columns=['movieId'], inplace=True)
    recommendation_list = recommendation_list.values.tolist()
    recommendation_list = list(flatten(recommendation_list))
